...
impl Ensoniq {
    fn new(io: Pio<0x40>, dma_bug_addr: u32, init_ctrl: u32) -> impl PinInit<Self, Error> {
        try_pin_init!(Self {
            io,
            dma_bug_addr,
    ...
    fn inl(&self, offset: u32) -> u32 {
        self.io.try_read32(offset as usize).expect("ens1370: port I/O offset out of range")
    }

    fn outl(&self, val: u32, offset: u32) {
        self.io.try_write32(val, offset as usize).expect("ens1370: port I/O offset out of range")
    }

    fn outw(&self, val: u16, offset: u32) {
        self.io.try_write16(val, offset as usize).expect("ens1370: port I/O offset out of range")
    }
...
//
// IRQ handler (Arc<Ensoniq> implements Handler via blanket impl)
//
...
struct Ens1370Data<'card> {
    ...
    _irq: Arc<irq::Registration<Arc<Ensoniq>>>,
...
impl<'card> pci::Driver for Ens1370Driver<'card> {
    ...
    fn probe<'bound>(
        ...
        // Enable the device (works for I/O-BAR devices in QEMU).
        pdev.enable_device_mem()?;
        pdev.request_all_regions(c"ENS1370")?;

        let port = pdev.resource_start(0)? as u32;
        pdev.set_master();

        // SAFETY: BAR0 is a valid I/O port range, allocated by the PCI layer.
        let io = unsafe { Pio::<0x40>::new(port) };
        ...
        let chip_arc = Arc::<Ensoniq>::pin_init(
            Ensoniq::new(io, dma_bug_addr, init_ctrl),
            GFP_KERNEL,
        )?;

        chip_arc.chip_init();

        // Allocate an INTx IRQ vector.
        ...
        // Request IRQ. Ok() wraps chip_arc as impl PinInit<Arc<Ensoniq>, Error>.
        let irq_reg = Arc::<irq::Registration<Arc<Ensoniq>>>::pin_init(
            pdev.request_irq(
                vector,
                irq::Flags::SHARED,
                c"ENS1370",
                Ok::<Arc<Ensoniq>, Error>(chip_arc.clone()),
            ),
            GFP_KERNEL,
        )?;
        ...
        dev_info!(pdev, "ENS1370 sound card registered at port 0x{:x}\n", port);

        Ok(Ens1370Data { chip_arc, _ak4531: ak4531, _irq: irq_reg, _card: card, _pm: pm })
    }
}
