...
#[pin_data]
struct Intel8x0 {
    addr: Pio<0x100>,  // AC97 mixer registers (BAR0)
    bm:   Pio<0x40>,   // busmaster DMA registers (BAR1)
    #[pin]
...
impl Intel8x0 {
    ...
    fn addr_inw(&self, offset: u32) -> u16 {
        self.addr.try_read16(offset as usize).expect("intel8x0: AC97 I/O offset out of range")
    }
    fn addr_outw(&self, val: u16, offset: u32) {
        self.addr.try_write16(val, offset as usize).expect("intel8x0: AC97 I/O offset out of range")
    }

    // Busmaster register space (BAR1)
    fn bm_inb(&self, offset: u32) -> u8 {
        self.bm.try_read8(offset as usize).expect("intel8x0: busmaster I/O offset out of range")
    }
    fn bm_inw(&self, offset: u32) -> u16 {
        self.bm.try_read16(offset as usize).expect("intel8x0: busmaster I/O offset out of range")
    }
    fn bm_inl(&self, offset: u32) -> u32 {
        self.bm.try_read32(offset as usize).expect("intel8x0: busmaster I/O offset out of range")
    }
    fn bm_outb(&self, val: u8, offset: u32) {
        self.bm.try_write8(val, offset as usize).expect("intel8x0: busmaster I/O offset out of range")
    }
    fn bm_outl(&self, val: u32, offset: u32) {
        self.bm.try_write32(val, offset as usize).expect("intel8x0: busmaster I/O offset out of range")
    }
}
...
impl irq::Handler for Intel8x0 {
    fn handle(&self, _dev: &device::Device<device::Bound>) -> irq::IrqReturn {
        let sta = self.bm_inl(ICH_REG_GLOB_STA);
        if sta == 0xffff_ffff {
...
struct Intel8x0Data<'card> {
    ...
    _irq_reg: Arc<irq::Registration<Arc<Intel8x0>>>,
...
impl<'card> pci::Driver for Intel8x0Driver<'card> {
    ...
    fn probe<'bound>(
        ...
        pdev.request_all_regions(c"Intel8x0")?;
        let addr_base    = pdev.resource_start(0)? as u32; // BAR0: AC97 registers
        let bm_addr_base = pdev.resource_start(1)? as u32; // BAR1: busmaster registers
        pdev.set_master();

        // SAFETY: BAR0 and BAR1 are valid I/O port ranges allocated by the PCI layer.
        let addr = unsafe { Pio::<0x100>::new(addr_base) };
        let bm   = unsafe { Pio::<0x40>::new(bm_addr_base) };
        ...
        let vector = *vecs.start();

        // Request shared IRQ.
        let irq_reg = Arc::<irq::Registration<Arc<Intel8x0>>>::pin_init(
            pdev.request_irq(
                vector,
                irq::Flags::SHARED,
                c"intel8x0",
                Ok::<Arc<Intel8x0>, Error>(chip_arc.clone()),
            ),
            GFP_KERNEL,
        )?;
        ...
        pr_info!("intel8x0: registered at I/O 0x{:x}/0x{:x}\n", addr_base, bm_addr_base);
